...
- (CGFloat)preferredCellWidthAtIndex:(NSInteger)index {
    if (self.cellWidth == JXCategoryViewAutomaticDimension) {
        CGFloat width = 0;
        if (self.titleDataSource && [self.titleDataSource respondsToSelector:@selector(categoryTitleView:widthForTitle:index:)]) {
            width = [self.titleDataSource categoryTitleView:self widthForTitle:self.titles[index] index:index];
